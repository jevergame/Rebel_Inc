Shader "Ndemic/LuxeSpriteShader" {
	Properties {
		_MainTex ("First Texture", 2D) = "white" {}
		_SecondTex ("Second Texture", 2D) = "white" {}
		[Toggle(USE_SECOND_TEXTURE)] _UseSecondTexture ("Use Second Texture", Float) = 0
		[KeywordEnum(Horizontal, Vertical, Radial, Transparency)] _FillMode ("Fill Mode", Float) = 0
		_Fill ("Fill", Range(0, 1)) = 0.5
		_Color ("Color", Vector) = (1,1,1,1)
		_From ("From", Vector) = (1,1,1,1)
		_To ("To", Vector) = (1,1,1,1)
		[Toggle(FLIP)] _Flip ("Flip", Float) = 0
		[Toggle(GRAYSCALE)] _Grayscale ("Grayscale", Float) = 0
		_Pivot ("Pivot", Vector) = (0,0,0,0)
	}
	//DummyShaderTextExporter
	SubShader{
		Tags { "RenderType"="Opaque" }
		LOD 200

		Pass
		{
			HLSLPROGRAM
			#pragma vertex vert
			#pragma fragment frag

			float4x4 unity_ObjectToWorld;
			float4x4 unity_MatrixVP;
			float4 _MainTex_ST;

			struct Vertex_Stage_Input
			{
				float4 pos : POSITION;
				float2 uv : TEXCOORD0;
			};

			struct Vertex_Stage_Output
			{
				float2 uv : TEXCOORD0;
				float4 pos : SV_POSITION;
			};

			Vertex_Stage_Output vert(Vertex_Stage_Input input)
			{
				Vertex_Stage_Output output;
				output.uv = (input.uv.xy * _MainTex_ST.xy) + _MainTex_ST.zw;
				output.pos = mul(unity_MatrixVP, mul(unity_ObjectToWorld, input.pos));
				return output;
			}

			Texture2D<float4> _MainTex;
			SamplerState sampler_MainTex;
			float4 _Color;

			struct Fragment_Stage_Input
			{
				float2 uv : TEXCOORD0;
			};

			float4 frag(Fragment_Stage_Input input) : SV_TARGET
			{
				return _MainTex.Sample(sampler_MainTex, input.uv.xy) * _Color;
			}

			ENDHLSL
		}
	}
}